...
namespace Server.Leaderboards
{
    ...
    public class Leaderboard
    {
        ...
        public async Task IncrementScore(string username, int diff)
        {
            var result = await _client.UpdateAsync<ScoreRecord>(ud => ud.Script("ctx._source.score+=" + diff));
            if (!result.IsValid)
            {
                throw new InvalidOperationException(string.Format("Failed to perform IncrementScore query : '{0}'", result.ServerError));
            }
        }
